...
@foreach($properties as $property)
	<div class="form-group">
		...
		@switch($property->InputType->type)
			@case(0)
				<input class="form-control" name="{{ $property->name }}" type="{{ $property->name == 'Year' ? 'number' : 'text' }}" id="{{ $property->name }}" value="{{ old('$property->name', optional($robot->RobotInfos->firstWhere('property_id', $property->id))->content) }}" minlength="0" maxlength="255" placeholder="{{$property->description}}" required>
